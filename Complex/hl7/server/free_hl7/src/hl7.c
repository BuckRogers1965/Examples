...
int
hl7count (struct hl7_part_t *message, struct hl7_location_t * location) {
...
int
hl7delete (struct hl7_part_t *message, struct hl7_location_t *location){
...
int
hl7insert (struct hl7_part_t *message, struct hl7_location_t *location, char *string){
...
int
hl7set (struct hl7_part_t *message, struct hl7_location_t *location, char *string){

  /* this one is a little harder, you have to traverse the string to the
     given location, creating missing nodes as you go and then set the final
     portion to the given string. */
...
char *
hl7join (struct hl7_part_t *hl7_part, char *msg_string, char *separators){
  ...
  if (hl7_part->lower){
    msg_string = hl7join(hl7_part->lower, msg_string, &separators[1]);
  }

  if (hl7_part->next){
    msg_string = concat(msg_string, current_separator);
    msg_string = hl7join(hl7_part->next, msg_string, separators);
  }
...
void
hl7printlocation(struct hl7_location_t *location){
...
void
hl7rawprint(struct hl7_part_t *hl7_part, struct hl7_location_t *location,
                                 struct hl7_location_t * current, int state)
{
  /* recursively traverse tree, keep track of location and print out data */
  if (!(current->next)){
...
void
hl7print (struct hl7_part_t *message)
{
  struct hl7_location_t *location;

  /* setup some data structures and call the real function */

  if (!(location = (struct hl7_location_t *)
                                   malloc(sizeof(struct hl7_location_t)))){
    exit (1);
  }
...
void
hl7freelocation(struct hl7_location_t *location){
...
void
hl7free (struct hl7_part_t *hl7_part)
{

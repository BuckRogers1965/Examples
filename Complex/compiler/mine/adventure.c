...
int Propername () {
	...
	if (dictionary[pToken].type == PROPERNAME)
		return pToken;
	else
		return 0;
}
...
int Relativepronoun () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == RELATIVEPRONOUN)
		return pToken;
	else
		return 0;
}
...
int Preposition () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == PREP)
		return pToken;
	else
		return 0;
}
...
int Pronoun () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == PRONOUN)
		return pToken;
	else
		return 0;
}
...
int Article () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == ARTICLE)
		return pToken;
	else
		return 0;
}
...
int Noun () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == NOUN)
		return pToken;
	else
		return 0;
}
...
int Verb () {
	int pToken = GetParsedToken();
	if (dictionary[pToken].type == VERB)
		return pToken;
	else
		return 0;
}
...
int PrepositionPhrase () {
	Preposition();
	Object();
}

/*
Command           ::= Predicate { Object { PrepositionPhrase }}
PrepositionPhrase ::= Preposition Object
Predicate         ::= Verb [Adverb]
Object            ::= [Object AND] {Article} {Adjective} Noun
*/

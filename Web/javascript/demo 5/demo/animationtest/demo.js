...
if (window.innerHeight!=null && window.scrollMaxY!=null) {// Firefox
		...
		if (xWithScroll < window.innerWidth){
			xWithScroll = window.innerWidth;
		}
  	}

	var pageHeight=yWithScroll+'px';
	var pageWidth=xWithScroll+'px';
	...
	var x = gethighestzindex();
	dark.style.zIndex="" + (parseInt(x)+1);
...
function OpenBoxes(){
    ...
    var x = gethighestzindex();
    box3.style.zIndex="" + (parseInt(x)+1);

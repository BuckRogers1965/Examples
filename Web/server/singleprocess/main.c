...
void
Initialize(int *epfd, int *listenfd, struct epoll_event *ev){
...
void
Loop(int *epfd, int *listenfd, struct epoll_event *ev){
  ...
  {
    //usleep(8000);
    res = epoll_wait(*epfd, events, MAX_CLIENT, 10);
    for(i = 0; i < res; i++)
    {
...
int main (){
  ...
  Initialize(&epfd, &listenfd, &ev);
  Loop(&epfd, &listenfd, &ev);
  Shutdown();

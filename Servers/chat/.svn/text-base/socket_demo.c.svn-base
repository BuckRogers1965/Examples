...
int get_verb(int fd){
	...
	if (client[fd].needed>1) return 1;
		else return 0;
} /* end get_verb */
...
void parse(int fd){
	...
	get_verb(fd);
	if (client[fd].needed==2) get_noun(fd);
	if (client[fd].needed==3) get_noun(fd);
}
...
void execute(int fd){
	...
	switch (client[fd].token[0]){
		case T_MUMBLE:{
			write(fd,"I don't understand what you have typed.\nType help for instruction on this games grammer.\n\n",89);
				printf("\a");
			} break;
		case T_QUIT:{
		...
		case T_HELP:{
			...
			write(fd,"\n",1);
			write(fd,"You must type a verb followed by a noun and optionally a prepositional phrase.\n",79);
			write(fd,"\n",1);
			write(fd,"Examples are:\n",14);
			write(fd,"go north     go east     go south     go west     go up     go down           \n",79);
			write(fd,"\n",1);
			write(fd,"To quit the game type: quit\n",27);
			write(fd,"\n",1);
			write(fd,"To talk to those in the room with you, type:  say  hello, how are you today?\n",77);
	...
	if(location[client[fd].location].connections[client[fd].token[1]]!=0)
	client[fd].location=location[client[fd].location].connections[client[fd].token[1]];
			else write(fd,"You can't go that way.\n\n",24);
			display_location(fd);
...
int load_locations(){
	...
	if(fp=fopen("locations.load","r")){
		...
		for(i=0;i<y;i++){
			fgets(string,BUFFER,fp);
			z=atoi(string);
			if (i!=z) printf("room was to be %d, read %d\n",i,z);
			fgets(string,BUFFER,fp);
